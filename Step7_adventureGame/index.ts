...
class Player {
  ...
  usepotion() {
    if (this.potion > 0) {
      ...
      this.potion -= 1;
      console.log(
        `After drinking potion ${this.name} health increses to ${this.health}`
      );
    } else {
  ...
  flee() {
    ...
    if (chance < 60) {
      this.health -= 30;
      console.log(
        `You attempted to flee, but the ${chalk.red(
          "enemy"
        )} caught up with you and dealt 30 damage.`
      );
      return false;
    } else {
...
const askName = async () => {
  ...
  const playerName: string = answers.player_name;
  return story(playerName);
};

const story = async (playerName: string) => {
  const user = new Player(playerName);
  console.clear();
  chalkAnimation.rainbow(
    `Welcome, ${playerName}, to the mystical world of adventures and challenges.\n`
  );
  await sleep(2000);
  console.log(
    chalk.bold.cyanBright(
      `You find yourself in a dense forest, surrounded by tall, ancient trees.`
    )
  );
  await sleep(1500);
  console.log(
    chalk.bold.cyanBright(
      `As you venture deeper into the forest, you come across a mysterious cave entrance.`
    )
  );
  await sleep(1500);
  console.log(
    chalk.bold.cyanBright(
      `Curiosity gets the best of you, and you decide to explore the cave.\n`
    )
  );
  await sleep(1500);
  console.log(`${chalk.bold.yellow(
    `Inside the cave, you discover an old chest filled with potions and treasures.`
  )}
${chalk.bold.yellowBright(`You obtained ${chalk.bold("a health potion")}`)}\n`);
  await sleep(1500);
  console.log(
    chalk.bold.cyanBright(
      `Little did you know that your journey was just beginning.`
    )
  );
  await sleep(1500);
  console.log(
    chalk.bold.cyanBright(
      `As you exit the cave, a shadowy figure emerges from the darkness.`
    )
  );
  await sleep(1500);
  console.log(
    chalk.bold.red(
      `It's a ${chalk.redBright("dangerous enemy")} looking for a fight.`
    )
  );
  await sleep(1500);
  console.log(
    chalk.bold.cyanBright(
      `Prepare yourself, ${playerName}, for the battle of your life!`
    )
  );
  await sleep(1500);
...
------------------------------\n`);
  while (user.health >= 0 && enemy.health >= 0) {
    ...
    await sleep(1000);
    const choice: string = await choices();
    await sleep();
    switch (choice) {
      case "Attack": {
        ...
        if (user.health <= 0) {
          ...
          ${chalk.bold.yellow(
            `The ${enemy.name} droped 2 Bottle of health potions and 20 Gold coins`
          )}
          `);
          break;
        } else {
...
const choices = async (): Promise<string> => {
  ...
  const choice: string = answers.option;
  return choice;
};
const end = async (user: Player) => {
  const answers = await inquirer.prompt({
    name: "option",
    type: "list",
    message: "What would you like to do ?",
