...
class Student {
    ...
    balance;
    status;
    constructor(name, id, enrolledCourse) {
        this.name = name;
        this.id = id;
        this.status = "unpaid";
    ...
    payFees(paymentAmount) {
        this.balance -= paymentAmount;
        if (this.balance <= 0) {
            this.status = "paid";
            console.log(`Fees successfully paid. Remaining balance: $${Math.abs(this.balance)}`);
        }
        else {
...
async function payFees(student) {
