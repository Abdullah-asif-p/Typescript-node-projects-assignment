...
const currencies = async () => {
    ...
    const currency1 = userInput1.currency;
    ...
    const currency2 = userInput2["currency"];
    const amount = inputAmount.amount;
    currency1 == currency2
        ? (console.log(`You selected the same currency (${currency1}) to convert from and to.\n`),
            currencies())
        : convert(currency1, currency2, amount);
};
const convert = async (currency1, currency2, amount) => {
    ...
    console.log(`Conversion Result: ${amount} ${currency1} equals ${convertedAmount} ${currency2}`);
};
await startapp();
