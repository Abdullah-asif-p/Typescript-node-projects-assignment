...
const askAnswer = async () => {
    const inputAnswer = await inquirer.prompt({
        name: "ans",
        type: "list",
        message: chalk.bold.hex(`#9E3C0E`)("Choose 1 if you like to talk to other Choose 2 if you would rather keep to yourself"),
        choices: ["1", "2"],
    });
    const answer = inputAnswer.ans;
    return answer;
};
const main = async () => {
    let answer = Number(await askAnswer());
    let p = new Person();
    p.askquestion(answer);
    console.log(`Your are : ${p.getPersonality}`);
    let name = String(await askName());
    let s = new Student(name);
    console.log(chalk.bold.hex(`#EB8921`)(`\nYour name is: ${s.getName} and your are personality type is:${s.getPersonality}`));
};
main();
