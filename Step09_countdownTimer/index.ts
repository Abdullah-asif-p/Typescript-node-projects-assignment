...
const countdownTime = async () => {
  const action = await inquirer.prompt([
    {
      type: "input",
      name: "op",
      message: `Enter countdown duration format ${chalk.bold.hex(`#DD4124`)(
        `mins:seconds, 10:01`
      )}`,
      validate(input) {
  ...
  let time: string[] = String(action.op).split(":");
  let min: number = Number(time[0]);
  let seconds: number = Number(time[1]);
  start();
...
const start = async () => {
  ...
  console.log(
    chalk.bold.hex(`#71C562`)(
      `The starting time is  ${hours}:${minutes}:${seconds}`
    )
  );
};
...
const end = () => {
  ...
  console.log(
    chalk.bold.hex(`#71C562`)(
      `The Ending time is ${hours}:${minutes}:${seconds}`
    )
  );
};

const timer = async (min: number, second: number) => {
  const interval = setInterval(() => {
    ...
    process.stdout.write(
      chalk.bold.hex(`#EFC050`)(
        `${String(min).padStart(2, "0")}:${String(second)}`
      )
    );
    min == 0 && second == 0
      ? (process.stdout.clearLine(0),
